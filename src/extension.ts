...
export function activate(context: vscode.ExtensionContext) {
  ...
  commands.push(
    vscode.commands.registerCommand("chatsensei.setModel", async () => {
      await vscode.window
        .showQuickPick(
          ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo", "gpt-4o", "gpt-4-32k"],
          {
  ...
  commands.push(
    vscode.commands.registerCommand("chatsensei.query", () => {
      ...
      vscode.window.showInputBox({ placeHolder: placeHolder }).then((input) => {
  ...
  commands.push(
    vscode.commands.registerCommand("chatsensei.checkUsage", () => {
      vscode.env.openExternal(
        vscode.Uri.parse("https://platform.openai.com/account/usage")
      );
    })
  );
